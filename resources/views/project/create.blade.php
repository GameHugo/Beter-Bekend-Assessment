    ...
    <div class="w-full flex justify-center">
        <div class="flex flex-col items-center mt-16 w-full">
            @if(isset($project))
                ...
                <form action="{{ route('projects.update', $project) }}" method="post" class="mt-4 w-1/5 flex flex-col items-center">
                @method('PUT')
            @else
                <h1 class="text-2xl font-bold">Create project</h1>
                <form action="{{ route('projects.store') }}" method="post" class="mt-4 w-1/5 flex flex-col items-center">
            @endif
                @csrf
                <div class="w-full">
